...
const makeUpper = (arr:string[]):string[]=>{
return arr.map((v)=>v.toUpperCase())
}
// 2. 커피 이름과 가격을 매개변수로 주면 커피 이름은
//  캐피탈라이즈(첫글자만 대문자) 하고 가격은 10% 할인되고
//  shot은 2로 자동으로 붙혀서 나오는 함수 만들기
const coffee = (coffeename:string, price:number):{coffeename:string,price:number,shot:number}=>{
...
const word = (char:string):boolean=>!!char
